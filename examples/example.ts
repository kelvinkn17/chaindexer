import { Chaindexer } from "../src"; // In real usage this would be "chaindexer"
...
async function main() {
  const indexer = new Chaindexer({
    ...
    wsRpcUrl: "wss://mainnet.infura.io/ws/v3/YOUR_INFURA_KEY", // Replace with your WebSocket RPC URL
    contracts: [
      {
        name: "USDC",
  ...
  indexer.on("log", (data) => {
    ...
    if (data.parsedEvent.name === "Transfer") {
      ...
      // Differentiate between real-time and historical events
